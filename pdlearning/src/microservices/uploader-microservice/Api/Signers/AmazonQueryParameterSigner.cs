...
namespace Microservice.Uploader.Signers
{
    ...
    /// </summary>
    public class AmazonQueryParameterSigner : AmazonSignerBase
    {
        /// <summary>
            ...
            string awsSecretKey)
        {
            ...
            paramDictionary.Add(XAmzAlgorithm, AmazonHttpHelpers.UrlEncode($"{Scheme}-{Algorithm}"));
            paramDictionary.Add(XAmzCredential, AmazonHttpHelpers.UrlEncode($"{awsAccessKey}/{scope}"));
            paramDictionary.Add(XAmzSignedHeaders, AmazonHttpHelpers.UrlEncode(canonicalizedHeaderNames));

            // x-amz-date is now added as a query parameter, not a header, but still needs to be in ISO8601 basic form
            paramDictionary.Add(XAmzDate, AmazonHttpHelpers.UrlEncode(dateTimeStamp));
            ...
            keyedHashAlgorithm.Key = DeriveSigningKey(HmacSha256, awsSecretKey, Region, dateStamp, Service);
            ...
            var signature = keyedHashAlgorithm.ComputeHash(Encoding.UTF8.GetBytes(stringToSign.ToString()));
