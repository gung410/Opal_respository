...
{
    public class GetFullTextFilteredEntitiesSharedQuery : BaseSharedQuery
    {
        /// <summary>
        ...
        public IQueryable<T> BySearchText<T>(IQueryable<T> query, string searchText, params Expression<Func<T, string>>[] inFullTextSearchProps)
        {
        ...
        /// <returns>Filtered by seach text query.</returns>
        public IQueryable<T> BySearchWords<T>(IQueryable<T> query, string searchText, params Func<string, Expression<Func<T, bool>>>[] filterBySingleWordPredicateBuilders)
        {
            if (string.IsNullOrWhiteSpace(searchText))
            {
                return query;
        ...
        public IQueryable<T> ByFilter<T>(IQueryable<T> query, string fieldType, string[] values, string[] notApplicableItems)
        {
            var fullTextSearchField = $"{fieldType}{EntitiesConstants.FullTextSearchSuffix}";
            ...
            foreach (var item in values)
            {
                ...
                {
                    // Search for a 'Not Applicable' value
                    expression = r => EF.Functions.Contains(EF.Property<string>(r, fullTextSearchField), item);
                }
                else
                {
                    // Search for 'Not Applicable'
                    expression = r => EF.Property<string>(r, fullTextSearchField) == null;

                    // Search for item which have "Not Applicable" value selected
                    if (notApplicableItems != null && notApplicableItems.Length > 0)
                    {
                        foreach (var notApplicableItem in notApplicableItems)
                        {
                            var applicableItem = notApplicableItem;
                            expression = expression.Or(r => EF.Functions.Contains(EF.Property<string>(r, fullTextSearchField), applicableItem));
                        }
                    }
                }
