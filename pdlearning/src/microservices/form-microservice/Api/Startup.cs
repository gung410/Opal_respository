...
namespace Microservice.Form
{
    public class Startup
    {
        ...
        {
            dbContextResolver.InitDatabase<FormDbContext>(
        ...
        {
            var dailyCheckingJobOption = _configuration.GetSection("DailyCheckingJobOptions").Get<HangfireScheduleOption>();
            var weeklyCheckingJobOption = _configuration.GetSection("WeeklyCheckingJobScheduleOption").Get<HangfireScheduleOption>();

            TimeZoneInfo systemTimeZone = DateTimeHelper
                .GetSystemTimeZone(dailyCheckingJobOption.WindowTimezone, dailyCheckingJobOption.LinuxTimezone);

            RecurringJob.AddOrUpdate<IFormDailyCheckingJob>(
                    t => t.ExecuteTask(null),
                    dailyCheckingJobOption.ScanSchedule,
                    queue: HangFireQueueName,
                    timeZone: systemTimeZone);

            RecurringJob.AddOrUpdate<IFormWeeklyCheckingJob>(
                    t => t.ExecuteTask(null),
                    weeklyCheckingJobOption.ScanSchedule,
                    queue: HangFireQueueName,
                    timeZone: systemTimeZone);
        }
    }
