...
{
    [OpalConsumer("csl.events.follow.removed")]
    public class UserUnFollowConsumer : ScopedOpalMessageConsumer<UserFollowingMessage>
    {
        private readonly ILogger<UserUnFollowConsumer> _logger;

        public UserUnFollowConsumer(ILogger<UserUnFollowConsumer> logger)
        {
            _logger = logger;
        }
            ...
            IRepository<UserFollowing> userFollowingRepository)
        {
            ...
            {
                _logger.LogError($"UserUnFollowConsumer invalid data: {JsonSerializer.Serialize(message)}");
                return;
            }
