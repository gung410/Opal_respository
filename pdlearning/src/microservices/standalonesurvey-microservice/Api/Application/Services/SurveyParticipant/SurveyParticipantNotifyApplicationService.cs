...
{
    public class SurveyParticipantNotifyApplicationService : BaseApplicationService
    {
        private readonly WebAppLinkBuilder _webAppLinkBuilder;

        public SurveyParticipantNotifyApplicationService(
        ...
        private async Task PerformSendNotify(NotifyFormParticipantModel model, FormParticipantNotifyType notifyType)
        {
            ...
            {
                FormDetailUrl = _webAppLinkBuilder.GetStandaloneFormPlayerLink(model.FormOriginalObjectId, SubModule.Lna), // Note: participant is lna only.
