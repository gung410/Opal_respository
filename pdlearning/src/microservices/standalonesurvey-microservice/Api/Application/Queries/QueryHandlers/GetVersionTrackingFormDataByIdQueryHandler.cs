...
{
    public class GetVersionTrackingFormDataByIdQueryHandler : BaseQueryHandler<GetVersionTrackingFormDataByIdQuery, VersionTrackingSurveyDataModel>
    {
        ...
        protected override async Task<VersionTrackingSurveyDataModel> HandleAsync(GetVersionTrackingFormDataByIdQuery query, CancellationToken cancellationToken)
        {
            ...
            var formSections = await _formSectionRepository
                .GetAll()
                .Where(section => section.SurveyId == query.FormId)
                .ToListAsync();

            return new VersionTrackingSurveyDataModel(form, formQuestions, formSections);
        }
    }
}
