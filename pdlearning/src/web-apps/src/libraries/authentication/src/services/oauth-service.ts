...
import { LoginOptions, OAuthStorage, OidcDiscoveryDoc, ParsedIdToken, TokenResponse, UserInfo } from '../models/types';
import { OAuthErrorEvent, OAuthEvent, OAuthInfoEvent, OAuthSuccessEvent } from '../models/events';
import { Observable, Subject, Subscription, from, of, race } from 'rxjs';
import { ValidationHandler, ValidationParams } from '../helpers/validation-handler';
import { b64DecodeUnicode, base64UrlEncode } from '../helpers/base64-helper';
import { delay, filter, first, map, switchMap, tap } from 'rxjs/operators';

import { AuthConfig } from '../models/auth.config';
import { CryptoHandler } from '../helpers/crypto-handler';
...
export class OAuthService extends AuthConfig implements OnDestroy {
    ...
    this.events
      ...
      .subscribe(e => {
        ...
        if ((listenTo == null || listenTo === 'any' || event.info === listenTo) && shouldRunSilentRefresh) {
          ...
          this.refreshInternal(params, noPrompt);
        }
      });
  ...
  public refreshTokenSilently(params: object = {}, listenTo?: 'access_token' | 'id_token' | 'any', noPrompt = true): void {
    // Notes for developers: This method is cloned from another method for quickly usage purpose.
  ...
  public triggerRefreshTokenLogic(): void {
    this.eventsSubject.next(new OAuthInfoEvent('token_expires', 'id_token'));
  }
   ...
   */
  public loadDiscoveryDocumentAndTryLogin(options: LoginOptions = null, redirectUrl?: string): Promise<boolean> {
    return this.loadDiscoveryDocument().then(doc => {
      return this.tryLogin(options, redirectUrl);
    });
  }
  ...
  protected setupExpirationTimers(): void {
    if (this.silentRefreshByAccessTokenOnly && this.hasValidAccessToken()) {
  ...
  public loadDiscoveryDocument(fullUrl: string = null): Promise<object> {
    return new Promise((resolve, reject) => {
      ...
      return this.http.get<OidcDiscoveryDoc>(fullUrl).subscribe(
        doc => {
  ...
  protected waitForSilentRefreshAfterSessionChange() {
    this.events
      .pipe(
        filter(
          (e: OAuthEvent) => e.type === 'silently_refreshed' || e.type === 'silent_refresh_timeout' || e.type === 'silent_refresh_error'
        ),
        first()
      )
      .subscribe(e => {
  ...
  protected async createLoginUrl(state = '', loginHint = '', customRedirectUri = '', noPrompt = false, params: object = {}) {
    const that = this;
  ...
  public tryLoginCodeFlow(options: LoginOptions = null, redirectUrl: string = null): Promise<void> {
    ...
    if (code) {
      return new Promise((resolve, reject) => {
        this.getTokenFromCode(code, redirectUrl)
          .then(result => {
  ...
  private getTokenFromCode(code: string, redirectUrl: string = null): Promise<object> {
    let params = new HttpParams().set('grant_type', 'authorization_code').set('code', code);

    if (redirectUrl) params = params.set('redirect_uri', redirectUrl);
    else params = params.set('redirect_uri', AppGlobal.environment.authConfig.silentRefreshRedirectUri);
   ...
   * @ignore
   */
  public processIdToken(idToken: string, accessToken: string, skipNonceCheck = false): Promise<ParsedIdToken> {
   ...
   */
  public initLoginFlow(additionalState = '', params = {}) {
   ...
   * the auth servers login url.
   */
  public initCodeFlow(additionalState = '', params = {}): Promise<void> {
    return this.initCodeFlowInternal(additionalState, params);
  }

  private initCodeFlowInternal(additionalState = '', params = {}): Promise<void> {
    if (!this.validateUrlForHttps(this.loginUrl)) {
      throw new Error('loginUrl must use Http. Also check property requireHttps.');
    }

    return this.createLoginUrl(additionalState, '', null, false, params).then(url => {
      location.href = url;

      return Promise.reject('The current user is not logged. Redirecting to login page...');
    });
  }
