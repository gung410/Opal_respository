...
import { StandaloneSurveyQuestionEditorComponent } from './standalone-survey-question-editor.component';
import { StandaloneSurveySectionEditorComponent } from './standalone-survey-section-editor.component';

@Component({
  selector: 'standalone-survey-editor',
  templateUrl: './standalone-survey-editor.component.html'
})
export class StandaloneSurveyEditorComponent extends BaseFormComponent implements AfterViewInit {
  public readonly StandaloneSurveyQuestionType: typeof StandaloneSurveyQuestionType = StandaloneSurveyQuestionType;

  @ViewChildren(StandaloneSurveyQuestionEditorComponent) public questionEditors: QueryList<StandaloneSurveyQuestionEditorComponent>;
  @ViewChildren(StandaloneSurveySectionEditorComponent) public sectionEditors: QueryList<StandaloneSurveySectionEditorComponent>;

  @Input() public formData: StandaloneSurveyModel = new StandaloneSurveyModel();

  public _formSectionsQuestions: StandaloneSurveySectionsQuestions = new StandaloneSurveySectionsQuestions({
    formQuestions: [],
    formSections: []
  });
  ...
  @Output() public formSectionsQuestionsChange: EventEmitter<StandaloneSurveySectionsQuestions> = new EventEmitter<
    StandaloneSurveySectionsQuestions
  >();
  @Output() public formSectionsChange: EventEmitter<SurveySection[]> = new EventEmitter<SurveySection[]>();
  @Output() public formDataChange: EventEmitter<StandaloneSurveyModel> = new EventEmitter<StandaloneSurveyModel>();
  @Output() public formQuestionsDataChange: EventEmitter<SurveyQuestionModel[]> = new EventEmitter<SurveyQuestionModel[]>();
  ...
  public branchingOptionQuestions: SurveyQuestionModel[] = [];
  public orderedFormQuestionsData: SurveyQuestionModel[] = [];
  public uneditableFormQuestionsLength: number | undefined;
  public questionsDicByFormSection: Dictionary<SurveyQuestionModel[]>;
  public formSectionDic: Dictionary<SurveySection>;

  constructor(
    protected moduleFacadeService: ModuleFacadeService,
    private elementRef: ElementRef,
    private editorPageService: StandaloneSurveyEditorPageService
  ) {
  ...
  public updateBranchingOptions(): void {
    ...
    this.formSectionsQuestions.formSections.forEach(section => {
      const sectionQuestion = new SurveyQuestionModel();
      sectionQuestion.questionType = StandaloneSurveyQuestionType.Structure;
  ...
  public getOrderedQuestionsBySection(sectionId: string): SurveyQuestionModel[] {
  ...
  public updateSectionsQuestionsData(actionFn: (data: StandaloneSurveySectionsQuestions) => void): void {
  ...
  public onSectionQuestionDataChange(newData: SurveyQuestionModel): void {
  ...
  public onSectionChanged(newFormSection: SurveySection): void {
  ...
  public onQuestionChanged(newData: SurveyQuestionModel): void {
    this.updateSectionsQuestionsData(sectionsQuestions => {
  ...
  public formQuestionsDataTrackByFn(index: number, item: SurveyQuestionModel): string | SurveyQuestionModel {
    return item.id;
  }

  public onQuestionTemplateSelect(
    event: { type: StandaloneSurveyQuestionType; priority: number; minorPriority: number },
    formSectionId?: string
  ): void {
    if (event.type === StandaloneSurveyQuestionType.Structure) {
      const newSection: ISurveySection = {
  ...
  public selectQuestion(question: SurveyQuestionModel): void {
  ...
  protected onChanges(changes: SimpleChanges): void {
    if (changes.formSectionsQuestions && changes.formSectionsQuestions.previousValue !== changes.formSectionsQuestions.currentValue) {
      ...
      this.editorPageService.formAutoSaveInformer$.pipe(this.untilDestroy()).subscribe(isAutoSaveRequest => {
        if (
          isAutoSaveRequest &&
  ...
  private createNewQuestion(type: StandaloneSurveyQuestionType, priority: number, minorPriority: number): SurveyQuestionModel {
    const questionTitle = '';
    switch (type) {
      case StandaloneSurveyQuestionType.FillInTheBlanks:
        return SurveyQuestionModel.CreateNewFillInTheBlanksQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.LongText:
        return SurveyQuestionModel.CreateNewLongTextQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.ShortText:
        return SurveyQuestionModel.CreateNewShortTextQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.TrueFalse:
        return SurveyQuestionModel.CreateNewTrueFalseQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.SingleChoice:
        return SurveyQuestionModel.CreateNewSingleChoiceQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.MultipleChoice:
        return SurveyQuestionModel.CreateNewMultipleChoiceQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.DropDown:
        return SurveyQuestionModel.CreateNewDropDownQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.DatePicker:
        return SurveyQuestionModel.CreateNewDatePickerQuestion(this.formData.id, priority, minorPriority, questionTitle);
      case StandaloneSurveyQuestionType.DateRangePicker:
        return SurveyQuestionModel.CreateNewDateRangePickerQuestion(this.formData.id, priority, minorPriority, questionTitle);
      default:
        return undefined;
    }
  }
  ...
  private insertNewQuestion(
    ...
    const newQuestion = this.createNewQuestion(questionType, priority, minorPriority);
  ...
  private processQuestionsPriority(formQuestionData: SurveyQuestionModel[]): void {
    ...
    newFormQuestions = newFormQuestions.filter(question => question.questionType !== StandaloneSurveyQuestionType.Structure);
