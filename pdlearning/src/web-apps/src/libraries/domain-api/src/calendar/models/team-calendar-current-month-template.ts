...
export class CurrentMonthGanttTemplate {
  public create(start: Date, end: Date): void {
    return kendo.ui.GanttView.extend({
      name: 'current-month-gantt-template',

      options: {
        weekHeaderTemplate: kendo.template(
          "<span class=''>#=kendo.toString(start, 'dd MMM')# - #=kendo.toString(kendo.date.addDays(end, -1), 'dd MMM')#<span>"
        )
      },
