...
Player.PersistentStateStorage = function(organizationIdentifier) {
  ...
  if (!isNullOrUndefined(organizationIdentifier)) {
    this.organization = organizationIdentifier;
  } else {
  ...
  if (this.isLocalMode()) {
    // Add more information to organization identifier to support playing multiple package by digital content id.
    this.organization += '_' + window.ScormPlayer.initData.digitalContentId;

    var userId = window.ScormPlayer.initData.userId;
    if (userId) {
      this.organization += '_' + userId;
    }

    if (!this.storage.hasKey(this.storageKey)) {
      data['organizations'] = {};
      data['globalobjectives'] = {};
    } else {
      data = this.storage.getObjectItem(this.storageKey);
    }

    this.removeOrganizationsIfExpired(data);
  } else {
    if (!this.scormData || !this.scormData['globalobjectives']) {
      data['organizations'] = {};
      data['globalobjectives'] = {};
    } else {
      data = this.scormData;
  ...
  var org;
  if (!Object.keyExists(organizations, this.organization)) {
    org = {};

    if (this.isLocalMode()) {
      org.expirationTime = new Date().getTime();
    }

    organizations[this.organization] = org;
  } else {
    org = organizations[this.organization];
  }
  if (!Object.keyExists(org, 'resumeallidentifier')) {
    org['resumeallidentifier'] = null;
  }
  if (!Object.keyExists(org, 'suspendedglobalobjectives')) {
    org['suspendedglobalobjectives'] = null;
  }
  if (!Object.keyExists(org, 'statuses')) {
    org['statuses'] = {};
  }
  if (!Object.keyExists(org, 'cmi')) {
    org['cmi'] = {};
  }
  if (!Object.keyExists(org, 'adldata')) {
    org['adldata'] = {};
  }
...
Player.PersistentStateStorage.prototype = {
  scormData: null,
  storage: null,
  storageKey: 'SCORM_PLAYER',
  expirationTimeInMs: 7 * 24 * 60 * 60 * 1000, // Default expiration time is 7 days.
  organization: null,
  supportLocalStorage: false,
  saveScormData: function(data) {
    try {
      if (this.isLocalMode()) {
        this.storage.setObjectItem(this.storageKey, data);
      } else {
        this.scormData = data;
        window.ScormPlayer.data = data;
        window.ScormPlayer.sendScormData(data);
      }
      this.$2 = true;
    } catch ($0) {
  ...
  isLocalMode: function() {
    return window.ScormPlayer.initData.displayMode === 'local';
  },
  ...
  getScormData: function() {
    if (this.isLocalMode()) {
      return this.storage.getObjectItem(this.storageKey);
    } else {
      return this.scormData;
    }
  },
  removeOrganizationsIfExpired: function(data) {
    for (identifier in data.organizations) {
      var org = data.organizations[identifier];
      if (!org.expirationTime || (org.expirationTime && new Date().getTime() - org.expirationTime > this.expirationTimeInMs)) {
        delete data.organizations[identifier];
      }
    }
  },
  getAllStatus: function() {
    var $0 = this.getScormData();
    var $1 = $0['organizations'];
    var $2 = $1[this.organization];
    return $2['statuses'];
  },
...
Player.ContentPlayer = function(manifestPath, imsmanifest, isStorage, isScormAuto, isDebug) {
  ...
  if (!isNullOrUndefined($0) && $0.length === 1) {
    if (API_BASE.BaseUtils.getText($0[0]) === '1.2') {
      ...
      API_BASE.BaseUtils.getText($0[0])
        .toLowerCase()
        .indexOf('1.3') >= 0 ||
      API_BASE.BaseUtils.getText($0[0])
        .toLowerCase()
    ...
    if (
      ($1.attributes.getNamedItem('xmlns') != null &&
        API_BASE.BaseUtils.getText($1.attributes.getNamedItem('xmlns')) === 'http://www.imsproject.org/xsd/imscp_rootv1p1p2') ||
      ($1.attributes.getNamedItem('version') != null && API_BASE.BaseUtils.getText($1.attributes.getNamedItem('version')) === '1.1')
    ) {
      this.$2 = 1;
    }
  }
  ...
  if (this.$2 === 1) {
    ...
    this.$9 = new SCORM_1_3.ActivityTree(
      ...
      this.$17 != null
        ? this.$17.getResumeAllIdentifier() != null
          ? this.$17.getSuspendedGlobalObjectives()
...
Player.ContentPlayer.prototype = {
  ...
  $19: 0,
  $1A: function() {
    // Function to init main content (if Scorm has)
    ...
    if (isNullOrUndefined(this.$3)) {
      ...
      this.$3.setAttribute('allowfullscreen', 'true');
      this.$3.setAttribute('webkitallowfullscreen', 'true');
      this.$3.setAttribute('mozallowfullscreen', 'true');
      this.$3.onload = function() {
        if (window.ScormPlayer.onContentLoaded) {
          window.ScormPlayer.onContentLoaded();
        }
  ...
  $1B: function() {
    // Function to init navigation (if Scorm has)
    this.$7 = document.getElementById('navigationContainer');
    // Force remove navigationContainer element if it existed by previous action -> pupose: reinit create element
  ...
  $1E: function() {
    // Function to init tree (if Scorm has)
    this.$5 = document.getElementById('treeContainer');
    const parentNodeOfTreeContainer = document.getElementById('placeholder_treeContainer');
    if (isNullOrUndefined(this.$5) || (parentNodeOfTreeContainer && parentNodeOfTreeContainer.style.display === 'none')) {
      ...
      if (isNullOrUndefined($0)) {
        ...
        if (this.$9.isSingleItem()) {
          ...
          $0.style.display = 'block'; // placeholder_treeContainer need to show
        }
      }
          ...
          if ($p1_0 !== this.$9.getOrganization()) {
            ...
            if ($p1_0.isLeaf()) {
              $1_0 = $p1_0
                .getParent()
                .getData()
                .addNode($p1_0.getTitle(), $1_1, PlayerConfiguration.TreeLeafIcon);
            } else {
              $1_0 = $p1_0
                .getParent()
                .getData()
                .addNode($p1_0.getTitle(), $1_1);
            }
            if ($1_1 != null) {
              this.$18.add($1_0);
            }
      ...
      // check if placeholder_treeContainer is block => change to none
      if (parentNodeOfTreeContainer.style.display === 'block') {
        parentNodeOfTreeContainer.style.display = 'none';
      }
    }
  },
  $1F: function() {
    // Function to init debugger screen
    if (!this.$1) {
      return;
    }
  ...
  $22: function($p0, $p1) {
    if ($p1.get_eventType() === 5 || $p1.get_eventType() === 3 || $p1.get_eventType() === 4) {
      ...
      if ($p1.get_eventType() === 5) {
        ...
        this.$B = null;
        this.$D = null;
        // When unload, we still need to show blank.html in the iframe.
      ...
      if (this.$17 != null) {
        if (this.$2 === 1) {
          ...
          this.$17.saveState(
            $0.getADLCPData(),
            $0.getStoredStatuses(),
            $0.savedSuspendedActivity != null ? null : $0.isObjectivesglobaltoSystem() ? $0.getClonedGlobalObjectives() : null,
            $0.savedSuspendedActivity != null ? $0.savedSuspendedActivity.getIdentifier() : null,
            $0.savedSuspendedActivity != null ? $0.getClonedGlobalObjectives() : null
          );
        }
      }
    ...
    if (this.$2 === 2) {
      if ($0) {
        ...
        if (!isNullOrUndefined(this.$10) && !$1.contains('previous')) {
          this.$10.disabled =
            !$3.getParentSequencing().flow || $3.getParentSequencing().forwardOnly || !$2.isValidNavigationRequest('previous', false);
        }
  ...
  $28: function($p0) {
    if ($p0 != null) {
      ...
      if (
        !isNullOrUndefined(PlayerConfiguration.TreeActiveIcon) &&
        PlayerConfiguration.TreeActiveIcon.length > 0 &&
        !isNullOrUndefined(this.$D) &&
...
Run.ManifestByString = function(manifest) {
  ...
  if (isNullOrUndefined(Run.$1) || isNullOrUndefined(API_BASE.BaseUtils.getChildNodeByName(Run.$1, 'manifest'))) {
    ...
    window.ScormPlayer.onPackageInvalid();
    return;
  }
  Run.$4();
};
Run.$3 = function() {
  if (Run.$0.readyState === 4) {
    ...
    if (Run.$0.status < 200 || Run.$0.status > 299) {
      window.ScormPlayer.onPackageInvalid();
      return;
    }
...
//ADDITIONAL INTEGRATED SCRIPT FOR OPAL 2
