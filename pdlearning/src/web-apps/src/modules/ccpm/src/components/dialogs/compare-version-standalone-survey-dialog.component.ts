...
@Component({
  selector: 'compare-version-standalone-survey-dialog',
  templateUrl: './compare-version-standalone-survey-dialog.component.html'
})
export class CompareVersionStandaloneSurveyDialogComponent extends BaseComponent {
  ...
  public formQuestionOld: ISurveyQuestionModel[] = [];
  public formQuestionNew: ISurveyQuestionModel[] = [];
  ...
  private initCompareData(parameter: CompareFormParameter): Promise<[void, void]> {
    ...
    const getOldVersionTrackingData = this.formApiService
      .getSurveyDataByVersionTrackingId(this.oldVersionTrackingVm.id, true)
      .then(result => {
    ...
    const getNewVersionTrackingData = this.formApiService
      .getSurveyDataByVersionTrackingId(this.newVersionTrackingVm.id, true)
      .then(result => {
    ...
    parameter.oldComparisonFormViewModels.forEach((item, index) => {
      if (item.changeType === ItemChangeType.NoneChange) {
        ...
        item.formQuestions.forEach((question, priority) => {
          if (
            question.changeType.includes(QuestionChangeType.NoneChange) &&
  ...
  private canCheckCorrectAnswerCheckedChange(type: StandaloneSurveyQuestionType): boolean {
    return (
      type === StandaloneSurveyQuestionType.TrueFalse ||
      type === StandaloneSurveyQuestionType.MultipleChoice ||
      type === StandaloneSurveyQuestionType.DropDown ||
      type === StandaloneSurveyQuestionType.SingleChoice
    );
  }

  private canCheckOptionValueChanged(type: StandaloneSurveyQuestionType): boolean {
    return (
      type === StandaloneSurveyQuestionType.MultipleChoice ||
      type === StandaloneSurveyQuestionType.DropDown ||
      type === StandaloneSurveyQuestionType.SingleChoice ||
      type === StandaloneSurveyQuestionType.FillInTheBlanks
    );
  }

  private canCheckMediaChanged(type: StandaloneSurveyQuestionType): boolean {
    return type === StandaloneSurveyQuestionType.MultipleChoice || type === StandaloneSurveyQuestionType.SingleChoice;
  }

  private isFreeTextQuestion(type: StandaloneSurveyQuestionType): boolean {
    return type === StandaloneSurveyQuestionType.ShortText;
  }

  private isDatePicker(type: StandaloneSurveyQuestionType): boolean {
    return type === StandaloneSurveyQuestionType.DatePicker;
  }

  private isDateRangePicker(type: StandaloneSurveyQuestionType): boolean {
    return type === StandaloneSurveyQuestionType.DateRangePicker;
  }
  ...
  private buildSectionComparisonModel(formData: StandaloneSurveyDataModel): IComparisonFormViewModel[] {
  ...
  private buildComparisonQuestionViewModel(question: SurveyQuestionModel): IComparisonQuestionViewModel {
    return {
      id: question.id,
  ...
  private buildComparisonQuestionOptionViewModel(question: SurveyQuestionModel): IComparisonQuestionOptionViewModel[] {
  ...
  private isAnswerValueCorrect(optionValue: StandaloneSurveyQuestionAnswerSingleValue, question: SurveyQuestionModel): boolean {
    ...
    if (question.questionType === StandaloneSurveyQuestionType.MultipleChoice) {
  ...
  private isOptionValueCorrect(optionValue: StandaloneSurveyQuestionAnswerSingleValue, question: SurveyQuestionModel): boolean {
...
export interface IComparisonQuestionOptionViewModel {
  ...
  type: StandaloneSurveyQuestionOptionType | undefined;
