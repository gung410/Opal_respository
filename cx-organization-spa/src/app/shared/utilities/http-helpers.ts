...
class HttpHelperOptions {
  ...
  /**
   * By the default, the hide loader function is triggered when having error.
   * Override this property if you would like to handle it by yourself.
   */
  avoidHideLoaderWhenError?: boolean;
}
...
export class HttpHelpers {
  ...
  get<T>(
    path: string,
    objparams?: any,
    helperOptions?: HttpHelperOptions
  ): Observable<T> {
  ...
  async deleteAsync<T>(
    ...
    const deletePromise = this.delete<T>(
      path,
      body,
      objparams,
      helperOptions
            ...
            if (Array.isArray(item)) {
              if (item.length > 0) {
                item.forEach((element: any) => {
                  paramsClone = paramsClone.append(key + `[${index}]`, element);
                });
              }
            } else {
  ...
  private getHttpOptions(options: HttpHelperOptions, objparams?: any): {} {
    ...
    if (httpOptions.avoidIntercepterCatchError) {
      httpOptions.headers[AppConstant.httpRequestAvoidIntercepterCatchError] =
        'true';
    }
    if (httpOptions.avoidHideLoaderWhenError) {
      httpOptions.headers[AppConstant.httpRequestAvoidHideLoaderWhenError] =
        'true';
    }
