...
export class DomUtils {
  ...
  public static closest(
    ...
    return predicate(parentEl)
      ? parentEl
      : DomUtils.closest(parentEl, predicate);
  }
  ...
  public static findClosestVerticalScrollableParent(
    ...
    return DomUtils.closest(element, (p) => {
      ...
      return (
        ...
        (!checkContentOverflow || DomUtils.isContentVerticalOverflow(p))
      );
    });
  }

  public static isContentVerticalOverflow(
    element: HTMLElement | Element
  ): boolean {
  ...
  public static getElementChildsTotalHeight(
    element: HTMLElement | Element
  ): number {
    let result = 0;
    for (let i = 0; i < element.children.length; i++) {
  ...
  public static scrollToView(
    ...
    const nearestScrollContainerEl = DomUtils.findClosestVerticalScrollableParent(
      element,
      !force
    );
    DomUtils.scrollToChild(
      nearestScrollContainerEl != null ? nearestScrollContainerEl : window,
      element,
      !force
    );
  }
  ...
  public static scrollToChild(
    ...
    if (onlyIfNeeded) {
      if (childTopPosition < parentTopPosition) {
        scrollVerticalByDistance(
          parent,
          childTopPosition - parentTopPosition - margin
        );
      } else if (childBottomPosition > parentBottomPosition) {
        scrollVerticalByDistance(
          parent,
          childBottomPosition - parentBottomPosition + margin
        );
      }
    } else {
      scrollVerticalByDistance(
    ...
    function scrollVerticalByDistance(
      scrollContainer: HTMLElement | Element | Window,
      scrollDistance: number
    ): void {
      if (scrollContainer instanceof Window) {
        scrollContainer.scrollTo(
          scrollContainer.scrollX,
          scrollContainer.scrollY + scrollDistance
        );
      } else {
  ...
  public static findFirstElement(
    ...
    for (let i = 0; i < rootElement.children.length; i++) {
      ...
      if (elFilter(element)) {
        ...
        (isFindRecursively == null || isFindRecursively(element))
      ) {
  ...
  public static getElementContentRect(element: HTMLElement): ClientRect {
    ...
    const paddingLeft =
      style.paddingLeft != null ? parseFloat(style.paddingLeft) : 0;
    const paddingRight =
      style.paddingRight != null ? parseFloat(style.paddingRight) : 0;
    const paddingTop =
      style.paddingTop != null ? parseFloat(style.paddingTop) : 0;
    const paddingBottom =
      style.paddingBottom != null ? parseFloat(style.paddingBottom) : 0;

    const elementRect: ClientRect = Utils.assign(
      <ClientRect>{},
