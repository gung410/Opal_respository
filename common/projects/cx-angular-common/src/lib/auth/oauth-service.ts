...
export class OAuthService extends AuthConfig implements OnDestroy {
        ...
        return new Promise<object>((resolve, reject) => {
            if (this.jwksUri) {
                this.http.get(this.jwksUri).subscribe(
                    ...
                    err => {
                        this.logger.error('error loading jwks', JSON.stringify(err));
                        this.eventsSubject.next(
        ...
        this.events
            ...
            .subscribe(e => {
                if (e.type !== 'silently_refreshed') {
                  console.log('silent refresh did not work after session changed', JSON.stringify(e));
    ...
    protected async createLoginUrl(
        ...
        // if nonce already existed then reuse it
        const currentNonce = this._storage.getItem('nonce');
        const nonce = currentNonce ? currentNonce : await this.createAndSaveNonce();
        ...
        return new Promise((resolve, reject) => {
            ...
            this.http.post<TokenResponse>(this.tokenEndpoint, params, { headers }).subscribe(
                ...
                (err) => {
                    ...
                    const eventType: EventType = hasValidAccessToken ? 'token_refresh_error' : 'token_error';
                    this.eventsSubject.next(new OAuthErrorEvent(eventType, err));
                    reject(err);
                }
            );
        });
    ...
    public logOut(noRedirectToLogoutUrl = false): void {
        ...
        //remove PKCI_verifier force creating new when user did logout
    ...
    public createAndSaveNonce(): Promise<string> {
        // if nonce already created then reuse it
        const that = this;
    ...
    protected async createChallangeVerifierPairForPKCE(): Promise<[string, string]> {
        ...
        // reuse PKCI_verifier to avoid multiple tabs opening login failed
        let pkciVerifier = this._storage.getItem('PKCI_verifier');
        if (!pkciVerifier) {
            pkciVerifier = await this.createNonce();
        }

        const challengeRaw = await this.crypto.calcHash(pkciVerifier, 'sha-256');
        const challange = base64UrlEncode(challengeRaw);

        return [challange, pkciVerifier];
    }
}
