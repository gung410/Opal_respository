    ...
    public static class ApplicationBuilderExtentions
    {
        ...
        private static async void OnStarted()
        {
            QueueConfiguration queueConfiguration = new QueueConfiguration()
            {
                ...
                ExchangeType = "topic",
                Queue = "rating_aggregation_queue",
        ...
        {
            _logger.LogWarning($"Processor acknowledge message: At({DateTime.UtcNow.ToString("G")}) DeliveryTag({basicDeliverEventArgs.DeliveryTag}) Redelivered({basicDeliverEventArgs.Redelivered}) ThreadId({Thread.CurrentThread.ManagedThreadId})");
