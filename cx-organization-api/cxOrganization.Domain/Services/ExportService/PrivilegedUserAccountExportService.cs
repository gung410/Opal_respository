...
{
    public class PrivilegedUserAccountExportService : UserExportServiceBase<PrivilegedUserAccountInfo>, IExportService<PrivilegedUserAccountInfo>
    {
        public PrivilegedUserAccountExportService(
        ...
        protected override void AddDataToExportDataTable(DataTable dataTableUser, IEnumerable<PrivilegedUserAccountInfo> source, ExportOption exportOption)
        {
            var propertyInfos = typeof(PrivilegedUserAccountInfo).GetProperties();
            foreach (PrivilegedUserAccountInfo item in source)
            {
                ...
                for (int i = 0; i < dataTableUser.Columns.Count; i++)
                {
                    ...
                    var fieldValue = GetFieldValueFromPrivilegedUserAccountInfo(rowNumber, fieldName, item, propertyInfos, column.GetFormat(), exportOption);
        ...
        protected object GetFieldValueFromPrivilegedUserAccountInfo(int rowNumber, string fieldName, PrivilegedUserAccountInfo item,
