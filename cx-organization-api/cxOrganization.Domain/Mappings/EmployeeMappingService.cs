...
{
    public class EmployeeMappingService : UserMappingService
    {
        ...
        {
            if (user == null || user.ArchetypeId != (short)ArchetypeEnum.Employee)
            {
                return null;
            }
            ...
            var userDtoBase = new EmployeeDto
            {
                ...
                JsonDynamicAttributes = string.IsNullOrEmpty(user.DynamicAttributes)
                    ? null
        ...
        private UserEntity NewUserEntity(HierarchyDepartmentEntity parentHd, EmployeeDto empDto)
        {
            var userNew = new UserEntity
            {
                ...
                SSN = _userCryptoService.EncryptSSN(empDto.SSN),
