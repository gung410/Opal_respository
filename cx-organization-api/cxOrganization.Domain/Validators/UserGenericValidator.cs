...
{
    public class UserGenericValidator : IUserValidator
    {
        ...
        public UserGenericValidator(IUserRepository userRepository,
            IWorkContext workContext,
            ICustomerRepository customerRepository,
            IOwnerRepository ownerRepository,
            IHierarchyDepartmentRepository hierarchyDepartmentRepository, IConfiguration configuration)
        {
            ...
            int? currentCustomerId = null)
        {
            ...
            var userDto = (UserGenericDto)dto;
            ...
            {
                var regexSSN = _configuration["Appsettings:SSN_REGEX"];
                if (string.IsNullOrEmpty(regexSSN))
                {
                ...
                {
                    if (!Utilities.ValidateFormatSsn(userDto.SSN, regexSSN))
                    {
                        throw new CXValidationException(cxExceptionCodes.VALIDATION_SSN_IS_INVALID);
                    }
                }
            ...
            if (userDto.Identity.CustomerId != _workContext.CurrentCustomerId && _workContext.IsEnableFiltercxToken)
            {
            ...
            var userEntities = _userRepository.GetUsers(_workContext.CurrentOwnerId, userId, string.Empty, userDto.Identity.ExtId);
