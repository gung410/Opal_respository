/****** Object:  StoredProcedure [dbo].[prc_UserList_get]    Script Date: 5/27/2020 1:21:54 PM ******/
SET ANSI_NULLS ON
GO
...
/* --NOTE
--Parameter with prefix  'Multi' should be in format 'value1,value2,value3&&value4,value5' ,
  in each groyp value (separated by '&&') we use for oprator 'IN'
  between groups we use operator AND
  Sample @MultiUserTypeIdFilter='1,3,4&&5,6' we expected to get user has user type id in (1,3,4) and user type id (5,6)

--Paramter filter on result status type log @IdpNeedStatusTypeLogFilter, @IdpPlanStatusTypeLogFilter shoud be in format 'statustypeId1,statustypeId2|fromdate1|todate1&&statustypeId3,statustypeId4|fromdate2|todate2&&'
  in each groyp value (separated by '&&') we get user match with list status type id and status type changed date should be beetween fromdate and todate
   between groups we use operator AND
  Sample @IdpNeedStatusTypeLogFilter='1,2|2020-05-26 00:00:00|2020-05-26 23:59:59&&3,4|2020-05-27 00:00:00|2020-05-27 23:59:59': we expected to get user has idp need status type changed to (1,2) from 2020-05-26 00:00:00 to 2020-05-26 23:59:59 AND status type changed to (3,4) from 2020-05-27 00:00:00 to 2020-05-27 23:59:59

-- Paramneter @AgeRangeFilter should be in format 'agegroup1,agegroup2'
   agegroup format:'fromAge-toAge'
   We get user that has age match any agegroup
   Sample: '0-19,30-39': we get user has age (from 0 to 19) OR (from 30 to 39)) 

--Paramter @JsonDynamicAttributeFilter should be in format 'filterGroup1&&fiilterGroup2
  filtergroup format
	+single value json key: 'key=value1,value2'
	+array value json key: 'key[]=value1,value2'
  We use AND between filtyer group
  In each filter group, we filter on one json key that match any value
  Example: @JsonDynamicAttributeFilter='$.designation=1213485&&$.jobFamily[]=bc461658-b43a-11e9-8d38-0242ac120004,bc4a4a70-b43a-11e9-8441-0242ac120004': we expecte to get user has designation=1213485 AND jobFamily match any value bc461658-b43a-11e9-8d38-0242ac120004,bc4a4a70-b43a-11e9-8441-0242ac120004'

*/
-- =============================================
ALTER PROCEDURE [dbo].[prc_UserList_get]
	...
	@AgeRangeFilter VARCHAR(500)= null,
	@JsonDynamicAttributeFilter NVARCHAR(MAX) = null, 
	...
	@IdpNeedStatusTypeLogFilter VARCHAR(max)= null,
	@IdpPlanStatusTypeLogFilter VARCHAR(max)= null,
...
SET @IdpNeedStatusTypeLogFilter = NULLIF(@IdpNeedStatusTypeLogFilter,'')
SET @IdpPlanStatusTypeLogFilter = NULLIF(@IdpPlanStatusTypeLogFilter,'')
...
OR @IdpNeedStatusTypeLogFilter IS NOT NULL
...
OR @IdpPlanStatusTypeLogFilter IS NOT NULL
...
END
 ...
 DECLARE @userTempTbl TABLE(
	...
	IF(@IdpNeedStatusTypeLogFilter IS NOT NULL)
	BEGIN

		DECLARE @WhereOnIdpNeedStatusTypeLog NVARCHAR(MAX) =''

		SELECT @WhereOnIdpNeedStatusTypeLog=@WhereOnIdpNeedStatusTypeLog +' AND exists (SELECT 1 FROM AT.StatusLog sl WHERE rNeed.ExtID = sl.ResultExtId '
		+ (SELECT STRING_AGG(IIF(ValueType = 1 AND t.FilterValue !='', ' AND sl.StatusTypeID in (' + FilterValue+')', '') + IIF(ValueType = 2 AND FilterValue!='', ' AND sl.Created >= ''' + FilterValue + '''', '') + (IIF(ValueType = 3 AND FilterValue!='', +' AND sl.Created <= ''' + FilterValue + '''', '')), '')
			FROM (SELECT ParseValue FilterValue ,ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS ValueType FROM dbo.StringToArray(filterGroup.ParseValue, '|')) t)
		+ ')'
		FROM dbo.StringToArray(@IdpNeedStatusTypeLogFilter, '&&') AS filterGroup


		SET @WhereStatement = @WhereStatement + @WhereOnIdpNeedStatusTypeLog
	...
	IF(@IdpPlanStatusTypeLogFilter IS NOT NULL)
	BEGIN
		DECLARE @WhereOnIdpPlanStatusTypeLog NVARCHAR(MAX) =''		
		
		SELECT @WhereOnIdpPlanStatusTypeLog=@WhereOnIdpPlanStatusTypeLog +' AND exists (SELECT 1 FROM AT.StatusLog sl WHERE rPlan.ExtID = sl.ResultExtId '
	+ (SELECT STRING_AGG(IIF(ValueType = 1 AND t.FilterValue !='', ' AND sl.StatusTypeID in (' + FilterValue+')', '') + IIF(ValueType = 2 AND FilterValue!='', ' AND sl.Created >= ''' + FilterValue + '''', '') + (IIF(ValueType = 3 AND FilterValue!='', +' AND sl.Created <= ''' + FilterValue + '''', '')), '')
		FROM (SELECT ParseValue FilterValue ,ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS ValueType FROM dbo.StringToArray(filterGroup.ParseValue, '|')) t)
	+ ')'
	FROM dbo.StringToArray(@IdpPlanStatusTypeLogFilter, '&&') AS filterGroup

				
		SET @WhereStatement = @WhereStatement + @WhereOnIdpPlanStatusTypeLog
